        ...
        <table class="w-full">
          <thead>
            <tr class="bg-zinc-200 dark:bg-zinc-800 border-b border-amber-500/50">
              ...
              <th class="px-6 py-4 text-left text-xs font-semibold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">Promotion</th>
          ...
          <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
            <tr v-for="po in purchaseOrders" :key="po.po_id" class="hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors">
              <td class="px-6 py-4">
                <span class="inline-flex items-center px-2.5 py-1 bg-amber-500/20 border border-amber-500 text-amber-600 dark:text-amber-400 text-sm font-semibold rounded-md">
                  {{ po.po_number }}
                </span>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm text-zinc-700 dark:text-zinc-300">{{ po.supplier?.company_name || '-' }}</span>
                </div>
              </td>
              ...
              <td class="px-6 py-4">
                <span v-if="po.promotion_amount" class="text-sm font-semibold text-amber-600 dark:text-amber-500">
...
const getStatusClasses = (status: string) => {
  ...
  switch (status) {
    case 'Pending':
      return `${base} bg-amber-500/20 text-amber-400 border border-amber-500`;
    case 'Shipped':
      return `${base} bg-blue-500/20 text-blue-400 border border-blue-500`;
    case 'Received':
      return `${base} bg-emerald-500/20 text-emerald-400 border border-emerald-500`;
    case 'Cancelled':
      return `${base} bg-red-500/20 text-red-400 border border-red-500`;
    default:
