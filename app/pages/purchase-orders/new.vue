  ...
  <div class="p-3 sm:p-4 md:p-6 lg:p-8 min-h-screen bg-white">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-4 sm:mb-6 lg:mb-8">
        <NuxtLink to="/purchase-orders" class="inline-flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-zinc-600 hover:text-amber-600 no-underline mb-2 sm:mb-4">
          <UIcon name="i-lucide-arrow-left" class="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          Back to Purchase Orders
        </NuxtLink>
        <h1 class="text-lg sm:text-xl md:text-2xl font-semibold text-zinc-900">Create Purchase Order</h1>
        <p class="mt-0.5 sm:mt-1 text-xs sm:text-sm text-zinc-500">Create a new purchase order with multiple items</p>
      </div>
        ...
        <div class="bg-white border border-zinc-200 rounded-xl p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-5">
          ...
          <div class="md:hidden space-y-3">
            ...
            >
              ...
              <div v-if="getProductDescription(item.product_id)" class="text-xs text-zinc-500 bg-white rounded p-2 border border-zinc-200">
                {{ getProductDescription(item.product_id) }}
              </div>
