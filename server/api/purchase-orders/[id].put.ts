...
export default defineEventHandler(async (event) => {
  ...
  if (isProduction()) {
    ...
    await supabase
      ...
      .update({
        ...
        payment_attachment: body.payment_attachment ?? existing.payment_attachment,
        authorized_by: body.authorized_by ?? existing.authorized_by,
        authorized_signature: body.authorized_signature ?? existing.authorized_signature,
        authorization_date: body.authorized_by && !existing.authorized_by ? new Date().toISOString().split('T')[0] : existing.authorization_date,
  ...
  execute(`
    UPDATE PurchaseOrders SET
      ...
      payment_attachment = COALESCE(?, payment_attachment),
      authorized_by = COALESCE(?, authorized_by),
      authorized_signature = COALESCE(?, authorized_signature),
      authorization_date = COALESCE(?, authorization_date),
  ...
  `, [
    ...
    body.payment_method,
    body.payment_status,
    body.payment_attachment,
    body.authorized_by,
    body.authorized_signature,
